...
async function analyzeImage(buffer, text) {
  try {
    const normalizedText = text.charAt(0).toUpperCase() + text.slice(1).toLowerCase() ;
    const lang = languageCodes[normalizedText] ;

    if(lang === undefined) throw new Error("wrong language") ;
    const { data: { text: ocrResult } } = await Tesseract.recognize(buffer, lang);
